...
const int replicates    = 5;    //number of run for one matrix
...
int uncertainty   = 10;  //CHANGE FROM FIRST MODEL: now a beta distribution. 0 is the least uncertain dist, 10 is uniform.
...
void statistics()
{
	...
	total_coop+= (coop )/ (2.0 * interactions);
	total_heuristic+=sum_defacto_heuristic / popsize;
...
int main()
{
	...
	string filename = "output_" + to_string(uncertainty) + ".txt";
	output.open(filename);
	output << "coop\t" << "defacto_heuristics\n";
	...
	for(int m =0;m< num_mat;m++){
		...
		total_coop /= replicates;
		total_heuristic/=replicates;
		output << total_coop << "\t";
		output << total_heuristic << "\n";
	}
	output.close();
}
